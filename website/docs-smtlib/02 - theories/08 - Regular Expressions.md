...
`(re.comp r)`            | The complement of the regular language of `r` |
`(re.diff r1 r2)`        | The difference of the regular languages of `r1` and `r2`  |
'(re.xor r1 r2)'         | The xor (symmetric difference) of regular languages `r1` and `r2` |
`re.none`                | The regular expression rejecting every string (SMT-LIB standard; z3 alias `re.nostr`)  |
...
`(re.replace_re s r dst)`  | Replace left-most smallest occurrence matching `r` in `s` by `dst` (standard: `str.replace_re`)  |
`(re.replace_re_all s r dst)`  | Replace, traversing left-to-right, smallest matches, all occurrences matching `r` in `s` by `dst` (standard: `str.replace_re_all`)  |
