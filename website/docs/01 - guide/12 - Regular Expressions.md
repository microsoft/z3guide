...
`(re.replace_re s r dst)'  | Currently not supported: replace first occurrence matching `r` in `s` by `dst`  |
`(re.replace_re_all s r dst)'  | Currently not supported: replace first occurrence matching `r` in `s` by `dst`  |
