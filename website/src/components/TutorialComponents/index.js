...
function OutputToggle(props) {
    const { onClick } = props;
    return (<button className="button button--primary" onClick={onClick}>
            Run
        </button>);
}
...
function Output(props) {
    ...
    let z3DuoOutput;
    try {
        z3DuoOutput = JSON.parse(result.output);
    }
    catch (e) {
    ...
    const buildOutput = (model1, res1, model2, res2) => {
    ...
    const regularOutput = (<pre className={codeChanged ? styles.outdated : ""}>
            {success ? ("") : timeout ? (<span style={{ color: "red" }}>
                    {`--${language} timeout--`}
                </span>) : !result.output && !result.error ? ("Press 'Run' to run the sample") : (<span style={{ color: "red" }}>
...
function CustomCodeEditor(props) {
    ...
    const component = (<Container as="div" className={clsx(editable ? styles.editable : "", codeBlockContainerStyles.codeBlockContainer, ThemeClassNames.common.codeBlock, language && `language-${language}`)}>
            {editable ? (<MonacoEditor lang={language} code={input} disabled={!editable} onChange={onChange} readonly={readonly} githubRepo={githubRepo} className={codeBlockContentStyles.codeBlockContent}/>) : (<CodeEditor code={input} theme={prismTheme} disabled={!editable} key={String(isBrowser)} className={codeBlockContentStyles.codeBlockContent} onChange={onChange} language={language} prism={Prism} githubRepo={githubRepo} readonly={readonly} showLineNumbers={showLineNumbers}/>)}
        </Container>);
...
export default function CustomCodeBlock(props) {
    ...
    const onDidChangeCode = (code) => {
        setCurrCode(code);
    };
    return (<div>
            <CustomCodeEditor input={code} id={result.hash} showLineNumbers={showLineNumbers} onChange={onDidChangeCode} editable={editable || outputRendered} language={highlight} githubRepo={githubRepo} readonly={readonly}/>
            <>
                <div className={styles.buttons}>
                    {!isZ3Duo && !readonly && !editable && !outputRendered && (<OutputToggle onClick={onDidClickOutputToggle}/>)}
                    {(isZ3Duo ||
            (!readonly && (editable || outputRendered))) && (<RunButton isZ3Duo={isZ3Duo} onClick={onDidClickRun} runFinished={runFinished}/>)}
                </div>
                {outputRendered ? (<Output language={lang} codeChanged={codeChanged} result={output} statusCodes={statusCodes}/>) : (<div />)}
            </>
        </div>);
}
